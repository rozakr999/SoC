...
assign start_request = (start_sync | c_start) & sync;
...
always_ff @(posedge clk)
    if (!resetn | ack_clear_req)
        ack <= 1'b1;
    else if (ena && bit_cntr == 6'd2 && state != RD_RX_DATA)
        ack <= ~sda_in;
    else
        ack <= ack;
...
always_ff @(posedge clk)
    if (!resetn)
        data_out <= 8'd0;
    else if ((state == RD_RX_DATA) && (bit_cntr > 6'd3) && (bit_cntr[1:0] == 2'b01) && ena)
...
begin
    ...
    case(state)
        ...
        begin
            scl_out = ^bit_cntr[1:0];
            sda_out = (bit_cntr > 6'd3) || (byte_cntr == 4'd1); 
            if (!ack)
                next_state = IDLE;
...
assign s_busy = (state != IDLE | start_sync | c_start);
